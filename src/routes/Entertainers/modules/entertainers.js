...
// Constants
// ------------------------------------
export const REQUEST_POSTS = 'REQUEST_POSTS_ENTERTAINERS'
export const RECEIVE_POSTS = 'RECEIVE_POSTS_ENTERTAINERS'
...
export const receivePosts = (json) => ({
  type: RECEIVE_POSTS,
  posts_entertainers: json.map(item => item)
})
...
const shouldFetchPosts = (state) => {
  if (state.entertainers.items == null) {
    return true
  } else {
    return false
  }
  ...
  [RECEIVE_POSTS]: (state, action) => {
    switch (action.type) {
      case RECEIVE_POSTS:
        return {
          ...state,
          items: action.posts_entertainers,
        }
      default:
        return state
    }
